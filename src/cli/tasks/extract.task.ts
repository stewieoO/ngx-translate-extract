...
export class ExtractTask implements TaskInterface {
	...
	public execute(): void {
		...
		if (this.options.parent && fs.existsSync(this.options.parent)) {
			this.out(bold('Found parent file:'), this.options.parent);
			const parentValues = this.compiler.parse(fs.readFileSync(this.options.parent, 'utf-8'));
			for (const key of Object.keys(parentValues.values)) {
				if (extracted.values[key] != null) {
					this.out(yellow('Clear parent value:'), key);
					delete extracted.values[key];
				}
			}
		}

		if (this.options.ignore && fs.existsSync(this.options.ignore)) {
			this.out(bold('Found ignore file:'), this.options.ignore);
			const ignoreValues = this.compiler.parse(fs.readFileSync(this.options.ignore, 'utf-8'));
			for (const key of Object.keys(ignoreValues.values)) {
				if (extracted.values[key] != null) {
					this.out(yellow('Clear ignore value:'), key);
					delete extracted.values[key];
				}
			}
		}
